...
public class Raiden {
    ...
    if (qUsed == true){
      dmg = dmgMultiplier(this.qLevel, 
        ...
        * (initialQSlashMultiplier(this.qLevel) 
           + resolveMultiplierBonus(this.qLevel, resolveStacks(this.qLevel, teamEnergy)))/100;
  ...
  public double resolveStacks(int qLevel, double teamEnergy) throws ArrayIndexOutOfBoundsException {
  ...
  public double ca1(int qLevel, int skillLevel){
    ...
    return
      dmgMultiplier(qLevel, skillLevel, this.atk, 
  ...
  public double totalCADmg(int qLevel,int skillLevel){
    return
      ca1(qLevel, skillLevel) + ca2(qLevel, skillLevel);
  }
}
